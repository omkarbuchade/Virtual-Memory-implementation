...
void PendingInterrupt::setWhen(int newTime)
{
    when=newTime;
}
...
void Interrupt::resetPending(int newTime)
{
     ListIterator<PendingInterrupt *> *itr = new ListIterator <PendingInterrupt *>(pending); 
     while (!itr->IsDone())
     {
        PendingInterrupt *obj=itr->Item(); 
        //std::cout<<"Interrupt val: "<<obj->when<<"\n";
        obj->setWhen(newTime);
        //std::cout<<"Changed interrupt val: "<<obj->when<<"\n";
        itr->Next();
     }
}
