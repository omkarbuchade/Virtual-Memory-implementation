...
//    -x runs one or more user program
...
//    -q provide a time quantum for the scheduling algorith for setting interrupts
//
...
main(int argc, char **argv)
{
    ...
    // some command line arguments are handled her.
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
        UserProgName[size] = argv[i + 1];
        size++;
	    //userProgName = argv[i + 1];
	    i++;
	}

    else if (strcmp(argv[i], "-q") == 0) {  //quantum flag
	    ASSERT(i + 1 < argc);	    
        quantum =  argv[i + 1];
       // kernel->stats->setTimerTicks(std::atoi(quantum));
        quantumFlag = TRUE;
	    i++;
	}
    ...
    if (quantumFlag) {
        kernel->interrupt->resetPending(std::atoi(quantum));
        kernel->stats->setTimerTicks(std::atoi(quantum));
    }
    ...
    {
        std::cout<<"FileName: "<<UserProgName[i]<<"\n";
